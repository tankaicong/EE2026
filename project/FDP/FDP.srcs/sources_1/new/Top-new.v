...
//----------- MORPHOLOGY (ERODE/DILATE) ----------- //
    ...
    wire [3:0] user_choices;        // From CV settings
    wire signed [3:0] num_blocks;          // From CV settings

    wire B1_pixel_out, B2_pixel_out, B3_pixel_out, B4_pixel_out;
    wire [16:0] B1_addr_out, B2_addr_out, B3_addr_out, B4_addr_out;
    reg signed [3:0] B1_addr_off_col, B2_addr_off_col, B3_addr_off_col, B4_addr_off_col;
    reg signed [3:0] B1_addr_off_row, B2_addr_off_row, B3_addr_off_row, B4_addr_off_row;
...
// ----------- FILTER SELECTION MUX CONTROLS ----------- //
    ...
    reg [2:0] stage_preprocessing = 3'h0;
    always @(*) begin
        ...
        bmp_addr_off_col = -num_blocks;
        bmp_addr_off_row = -num_blocks;
        Last_Stage <= stage_preprocessing + num_blocks;
        // if (user_choices[num_blocks] == 0 && num_blocks > 0) Last_Stage = 3'd5;
        // if (user_choices[num_blocks] == 1 && num_blocks > 0) Last_Stage = 3'd4;
        BMP_Out_Control <= num_blocks;
...
// ------------- CV Settings: Drag & Drop/Change View/Infotab for Preprocessing/Morphology --------------
    ...
    cv_settings_dragdrop settings_cv (
        ...
        .morph_count(num_blocks),
        .morph_vector(user_choices),
        // .morph_count(morph_count),
...
// ----------- MOUSE CONTROLLER AND SERVO ----------- //
    ...
    wire [3:0] mouse_z_raw;
    wire [3:0] zpos;
    assign mouse_z_raw = zpos;
    assign led[15:12] = mouse_z_raw; // show scroll wheel value on LEDs
    assign led[11:9] = num_blocks; // show final output selection on LEDs
